...
[chag](https://raw.githubusercontent.com/mtdowling/chag/master/install.sh) is used to keep annotated tags and the CHANGELOG in sync.
...
Development is done in Visual Studio Code and the [devcontainer setup](https://code.visualstudio.com/docs/remote/containers).
...
FROM --platform=linux/amd64 <projectname>-cmk-python3:${VARIANT}
...
During this step, all relevant files get symlinked into the version specific folder of the CMK Docker container.
...
* Run `.devcontainer/scripts/create_dummyhost.sh`:
  * creates a host
  * adds a bakery rule
